document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Popups Setup and Listeners
    function setupPopupListener() {
        const popupForm = document.getElementById('form-popup');
        if (!popupForm) return;

        const pEnabled = document.getElementById('popup-enabled');
        const pTitle = document.getElementById('popup-title');
        const pMessage = document.getElementById('popup-message');
        const pBtnText = document.getElementById('popup-btn-text');
        const pBtnLink = document.getElementById('popup-btn-link');
        ...
        // Live Preview Logic
        const updatePreview = () => {
            prevTitle.innerText = pTitle.value || 'Preview Title';
            prevMsg.innerText = pMessage.value || 'Your message will appear here. Keep it engaging!';
            prevBtn.innerText = pBtnText.value || 'Button Text';
        };

        pTitle.addEventListener('input', updatePreview);
        pMessage.addEventListener('input', updatePreview);
        pBtnText.addEventListener('input', updatePreview);

        // Fetch initial popup data
        db.collection('settings').doc('popup').get().then((doc) => {
            if (doc.exists) {
                const data = doc.data();
                pEnabled.checked = data.enabled || false;
                pTitle.value = data.title || '';
                pMessage.value = data.message || '';
                pBtnText.value = data.btnText || '';
                pBtnLink.value = data.btnLink || '';
                updatePreview();
            }
        ...
        popupForm.addEventListener('submit', async (e) => {
            ...
            try {
                await db.collection('settings').doc('popup').set({
                    ...
                    title: stripHTML(pTitle.value.trim()),
                    message: stripHTML(pMessage.value.trim()),
                    btnText: stripHTML(pBtnText.value.trim()),
                    btnLink: stripHTML(pBtnLink.value.trim()),
                    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
                });
